...
- `local-server/` — `adant-local`, the FastMCP stdio server owning
  ...
  panel. The progress panel and the media preview are built with the official
  MCP Apps SDK into content-hashed single-file assets; the progress panel also
  has a tokenized 127.0.0.1 HTTP fallback. The POSIX launcher locates `uv`;
  `uv` provisions Python and the frozen production environment, so users do
