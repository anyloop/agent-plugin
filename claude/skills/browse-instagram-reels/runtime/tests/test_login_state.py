...
def test_login_state_requires_live_instagram_session(tmp_path, monkeypatch):
    monkeypatch.setattr(browse, "CDP_PROFILE_DIR", tmp_path)
    assert browse._check_instagram_login() is False

    _write_cookie(tmp_path)
    assert browse._check_instagram_login() is True


def test_login_state_rejects_user_id_expired_or_wrong_domain_cookie(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(browse, "CDP_PROFILE_DIR", tmp_path)
    _write_cookie(tmp_path, name="ds_user_id")
    assert browse._check_instagram_login() is False

    expired_profile = tmp_path / "expired"
    monkeypatch.setattr(browse, "CDP_PROFILE_DIR", expired_profile)
    _write_cookie(expired_profile, expires_offset=-60)
    assert browse._check_instagram_login() is False

    wrong_profile = tmp_path / "wrong-domain"
    monkeypatch.setattr(browse, "CDP_PROFILE_DIR", wrong_profile)
    _write_cookie(wrong_profile, host="notinstagram.com")
    assert browse._check_instagram_login() is False
...
def test_login_state_is_unknown_when_cookie_store_is_unreadable(tmp_path, monkeypatch):
    ...
    assert browse._check_instagram_login() is None
...
def test_visible_login_keeps_direct_launch_fallback_outside_macos(
    ...
    browse._launch_visible_login_browser(
...
def test_browser_ownership_requires_exact_process_arguments():
    ...
    port = "--remote-debugging-port=9334"
...
def test_owned_pid_requires_chrome_and_exact_profile(tmp_path, monkeypatch):
    ...
    port_arg = "--remote-debugging-port=9334"
    commands = "\n".join(
        [
